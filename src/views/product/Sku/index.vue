<template>
  <div>
    <!--    表格区-->
    <el-table v-loading="loading" :data="pagination.records" ref="singleTable" border highlight-current-row
      style="width: 100%">
      <el-table-column align="center" type="index" label="序号" width="80"/>
      <el-table-column align="center" prop="skuName" label="名称">
      </el-table-column>
      <el-table-column align="center" prop="skuDesc" label="描述">
      </el-table-column>
      ...
      <el-table-column align="center" label="操作" width="240">
        <template slot-scope="{ row, $index }">
          ...
          <el-button type="primary" icon="el-icon-edit" size="mini" title="正在开发中！" @click="$message('正在开发中！')">
          </el-button>
          <el-button type="info" icon="el-icon-info" size="mini" title="查看Sku信息" @click="SkuById(row, $index)">
          </el-button>
          <el-popconfirm style="margin-left: 10px" :title="`你确定要删除：${row.skuName}`"
    ...
    <!--    分页区-->
    <el-pagination align="center" @current-change="CurrentChange" @size-change="SizeChange" :pager-count="5"
      :page-sizes="[10, 30, 60, 100]" :current-page="this.pagination.page" :page-size="this.pagination.limit"
      layout="prev, pager, next, jumper,->,total, sizes" :total="this.pagination.total">
    </el-pagination>
    <!--    抽屉区-->
  ...
  methods: {
    ...
    async PutOrDown({ isSales, row }) {
      if (isSales === 0) {
        this.loading = true;
        row.isSale = 0;
        let result = await this.$API.sku.reqPutOrDown({
          isSale: 0,
          skuId: row.id,
        });
        if (result.code === 200) {
